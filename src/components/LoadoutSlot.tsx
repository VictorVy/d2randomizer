...
const LoadoutSlot = ({ item }: LoadoutSlotProps) => {
    return (
        <div className="relative rounded-md border-2 border-neutral-950 border-opacity-30 bg-black bg-opacity-25">
            {item ? (
                <>
                    <img src={"https://bungie.net" + item.icon} width={50} height={50} className="peer rounded-md" />
                    <span
                        className="absolute -top-[2.8em] left-1/2 min-w-max origin-bottom -translate-x-1/2 scale-0
                rounded-md bg-gray-900 bg-opacity-80
                px-2 py-1 text-[0.6em]
