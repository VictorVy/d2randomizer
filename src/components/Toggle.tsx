...
const Toggle = ({ text, defaultCheck, forWeapons, disabled, onChange }: ToggleProps) => {
    return (
        <div className={"relative flex" + (forWeapons ? " justify-end" : "")}>
            {forWeapons && (
                <label
                    className={
                        "inline-block -translate-y-[0.08em] cursor-pointer pr-[0.35rem] text-sm text-white opacity-90 duration-150 hover:opacity-100"
                    }
            ...
            {!forWeapons && (
                <label
                    className={
                        "inline-block -translate-y-[0.08em] cursor-pointer pl-[0.15rem] text-sm text-white opacity-90 duration-150 peer-hover:opacity-100"
                    }
                    htmlFor={"toggle-" + text + (forWeapons ? "-weapons" : "-armour")}
                >
                    {text}
